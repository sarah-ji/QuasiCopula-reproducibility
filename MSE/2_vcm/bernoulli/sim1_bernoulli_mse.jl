...
function runtest()
    ...
    isdir("bernoulli_vc1") || mkdir("bernoulli_vc1")

    writedlm("bernoulli_vc1/mse_beta_" * ftail, βMseResults, ',')
    writedlm("bernoulli_vc1/mse_theta_" * ftail, θMseResults, ',')
    writedlm("bernoulli_vc1/fittimes_" * ftail, fittimes, ',')

    writedlm("bernoulli_vc1/beta_theta_coverage_" * ftail, βθcoverage, ',')

    # glmm
    writedlm("bernoulli_vc1/mse_beta_GLMM_" * ftail, βMseResults_GLMM, ',')
    writedlm("bernoulli_vc1/mse_theta_GLMM_" * ftail, θMseResults_GLMM, ',')
    writedlm("bernoulli_vc1/fittimes_GLMM_" * ftail, fittimes_GLMM, ',')
end
runtest()
