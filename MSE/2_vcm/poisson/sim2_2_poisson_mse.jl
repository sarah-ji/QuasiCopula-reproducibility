...
function runtest()
    ...
    for t in 1:length(samplesizes)
        ...
        for k in 1:length(ns)
            ...
            Γ = θtrue[1] * ones(ni, ni) + 0.000000000000001 * Matrix(I, ni, ni)
            for j in 1:nsims
    ...
    # make sure poisson_vc2_2 is a directory
    isdir("poisson_vc2_2") || mkdir("poisson_vc2_2")

    writedlm("poisson_vc2_2/mse_beta_" * ftail, βMseResults, ',')
    writedlm("poisson_vc2_2/mse_theta_" * ftail, θMseResults, ',')
    writedlm("poisson_vc2_2/fittimes_" * ftail, fittimes, ',')

    writedlm("poisson_vc2_2/beta_theta_coverage_" * ftail, βθcoverage, ',')

#     # glmm
    writedlm("poisson_vc2_2/mse_beta_GLMM_" * ftail, βMseResults_GLMM, ',')
    writedlm("poisson_vc2_2/mse_theta_GLMM_" * ftail, θMseResults_GLMM, ',')
    writedlm("poisson_vc2_2/fittimes_GLMM_" * ftail, fittimes_GLMM, ',')
end
runtest()
